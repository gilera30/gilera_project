...
app.get("/lk", isAuth, async (req, res) => {
  ...
  if (isAdmin) {
    ...
    const courses = await Course.findAll({
      include: [{ model: Record, where: { user_id: req.session.user.id } }],
    });
...
app.get("/admin", isAdmin, async (req, res) => {
  const usersAll = await User.findAll({
    include: [
      { model: Qualification, attributes: ["name"] },
  ...
  res.render("admin.hbs", {
    ...
    records: recordsAll,
    courses: courseAll,
    qualifications: qualificationAll,
  });
});
...
app.post("/register", async (req, res) => {
  const { name, phone, login, password, repeat_password, qualification_id } =
    req.body;
...
(async () => {
  ...
  const qualification3 = await Qualification.create({
